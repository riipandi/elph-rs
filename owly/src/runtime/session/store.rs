...
impl SessionStore {
    pub async fn open(cwd: &Path) -> Result<Self> {
        Self::open_with_path(crate::runtime::checkpoint::default_db_path(), cwd).await
    }
...
mod tests {
    use super::*;
    use crate::runtime::session::{MESSAGES_CHANNEL, interactive_config, tool_write_channel};
