use crate::{
    ...
    AnyElement, CanvasTextStyle, Color, Component, ComponentDrawer, ComponentUpdater, FlexDirection, Hook, Hooks,
    JustifyContent, KeyCode, KeyEvent, KeyEventKind, MouseEventKind, Overflow, Position, Props, TerminalEvent,
};
...
impl ScrollViewHandle {
    ...
    pub fn scroll_to(&mut self, offset: i32) {
        if let Some(inner) = &mut self.inner {
            inner
                .scroll_offset
                .set(clamp_offset(offset, inner.content_height.get(), inner.viewport_height.get()));
            Self::update_user_scrolled_up(inner);
        }
    }
    ...
    pub fn scroll_offset(&self) -> i32 {
        self.inner.as_ref().map_or(0, |inner| inner.scroll_offset.get())
    }

    /// Returns the total height of the scrollable content in lines.
    pub fn content_height(&self) -> u16 {
        self.inner.as_ref().map_or(0, |inner| inner.content_height.get())
    }

    /// Returns the height of the visible viewport in lines.
    pub fn viewport_height(&self) -> u16 {
        self.inner.as_ref().map_or(0, |inner| inner.viewport_height.get())
    }

    /// Returns whether auto scroll is currently pinned to the bottom.
    pub fn is_auto_scroll_pinned(&self) -> bool {
        self.inner.as_ref().is_none_or(|inner| !inner.user_scrolled_up.get())
    }
...
/// ```
#[component]
pub fn ScrollView<'a>(mut hooks: Hooks, props: &mut ScrollViewProps<'a>) -> impl Into<AnyElement<'a>> {
    ...
    let h = hooks.use_hook(move || MeasureHeightHook { out: viewport_height });
    ...
    if !pinned_to_bottom {
        let clamped = clamp_offset(scroll_offset.get(), content_height.get(), viewport_height.get());
    ...
    if pinned_to_bottom {
        ...
        element! {
            View(width: 100pct, height: 100pct, flex_direction: FlexDirection::Row) {
                View(overflow: Overflow::Hidden, flex_grow: 1f32, height: 100pct) {
    ...
    async fn test_scroll_view_basic_render() {
        let canvases: Vec<_> = element!(TestScrollView)
            .mock_terminal_render_loop(MockTerminalConfig::with_events(stream::iter(vec![TerminalEvent::Key(
                KeyEvent::new(KeyEventKind::Press, KeyCode::Char('q')),
            )])))
            .collect()
            .await;
